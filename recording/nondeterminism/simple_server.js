...
function extract_id_cookie(cookie) {
    var key = "Client-XHR-ID";
    var cookies = cookie.split(";");
    for(var i = 0; i < cookies.length; i++){
        var tokens = cookies[i].split("=");
        if(tokens[0] == key || tokens[0] == " Client-XHR-ID"){
            return tokens[1];
        }
    }
    return "None";
...
const server = http.createServer(function(request, response) {
    ...
    request.on('data', function(chunk) {
     ...
     if ( req_cookie ) {
        ...
        var xhr_id = extract_id_cookie(req_cookie);
        console.log("GOT XHR ID: " + xhr_id);
     }
